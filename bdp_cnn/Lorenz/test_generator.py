...
model = Sequential()
model.add(LSTM(100,batch_size=31,stateful=False,input_shape=(7,40)))
model.add(Dense(40))
...
model.fit_generator(data_gen,shuffle=False,epochs=1,verbose=1,validation_data=valid_gen)


trained_weights = model.get_weights()

pred_model = Sequential()
pred_model.add(LSTM(units=100,batch_size=1,stateful=False,input_shape=(7,40)))
pred_model.add(Dense(40))
pred_model.set_weights(trained_weights)
pred_model.compile(loss='mean_squared_error', optimizer='adam')

test_gen = TimeseriesGenerator(data,targets,length=7,sampling_rate=1,batch_size=1)
