...
class Evaluater(object):
    ...
    def map_mae(self, ytest, ypred, neurons, batch_size, epochs, time_steps, runtime, path="./"):
        ...
        plt.savefig(path+"LSTM_maemap_"
                    ...
                    +str(epochs)+"epochs_"
                    +str(time_steps)+"timesteps_pr.png", dpi=400)

    def map_mae_step(self, ytest, ypred, neurons, batch_size, epochs, time_steps, runtime, path="./"):
        """
        Plots the mean absolute error between the truth and prediction on the map.
        ...
        if not os.path.isdir(path + "step"):
            os.mkdir(path + "step")
        else:
            os.system("rm -r %sstep"%path)
            os.mkdir(path + "step")
        ...
        for mae in diff:
            ...
            levels = np.linspace(-5.25, 5.25, 20, endpoint=True)
            ticks = np.linspace(-5, 5, 11, endpoint=True)
            ...
            cp = plt.imshow(mae.T, cmap=plt.cm.get_cmap("seismic", 11), vmin=-5.25, vmax=5.25, extent=[0.,358.125,-88.57216851,88.57216851],
                       interpolation='none')
            cb = plt.colorbar(cp, extend="both", ticks=ticks)
            cb.set_label(r'LSTM Temperature - CMIP5 Temperature ($\Delta{}$K)')
            ...
            plt.savefig(path+"step/LSTM_maemap_"
                        +str(neurons)+"neurons_"
                        +str(batch_size)+"batchsize_"
                        +str(epochs)+"epochs_"
                        +str(time_steps)+"timesteps_%i.png"%counter, dpi=400)
            plt.close()
            counter += 1
            if counter >= 30:
                break
...
if __name__ == "__main__":
    ...
    folder = '20180518_1222_41s'

    path = "/home/tobias/Documents/bdp_runs/" + folder + '/'
    file = glob.glob(path + 'RMSE*.nc')[0].split("/")[-1]
    ev = Evaluater()
    dh = DataHandler()
    ...
    ev.map_mae_step(trues, preds, neurons, batch_size, epochs, time_steps, runtime, path=path)
